...
public class NavAgentAnimatorSync : MonoBehaviour
{
    ...
    public AudioSource audioSource; // Add for voice playback
    public AudioClip[] voiceClips; // Array for multiple voice clips
    public int currentClipIndex = 0; // Index to select which clip to play
    ...
    void Update()
    {
        ...
        if (isCloseToDestination && !agent.isStopped)
        {
            ...
            // Play the current voice clip if available
            if (audioSource != null && voiceClips.Length > currentClipIndex && voiceClips[currentClipIndex] != null)
            {
                audioSource.PlayOneShot(voiceClips[currentClipIndex]);
                currentClipIndex++; // Increment for next arrival
            }
        }
